(function (global, factory) {
    ...
    EventHandler.delegate(container, 'mouseover mouseout', '.jvm-element', function (event) {
      ...
      if (event.type === 'mouseover') {
        ...
        if (!event.defaultPrevented) {
          if (showTooltip) {
            if (data.element.constructor.name == 'Marker') { //modifier par moi pour n'afficher les labels que des markers et non pas des régions
              map.tooltip.show();
            };
          }
        }
  ...
  function setupZoomButtons() {
    ...
    var map = this;
    var zoomin = createElement('div', 'jvm-zoom-btn jvm-zoomin', '<span>+</span>', true);
    var zoomout = createElement('div', 'jvm-zoom-btn jvm-zoomout', '<span>−</span>', true);
    var france = findElement(this.container, '.jvm-france');
    ...
    if (window.innerWidth < 600) {
      //permet de zoomer dans les affichages mobiles pour que ce soit plus clair que la carte est intéractive
      function zoom_mobile() {
        var labels_doc = document.querySelectorAll('.jvm-tooltip');
        if (labels_doc.length < 1) {
          //console.log('En cours de chargement...');
          setTimeout(() => {
            zoom_mobile();
          }, 1000);
        } else {
          _this._setScale(1.5, map.width / 2, map.height / 2, false, map.params.zoomAnimate);
        };
      };
      zoom_mobile();
    ...
    /*EventHandler.on(france, 'click', function () {

      _this._setScale(2.5, map.width / 2, map.height / 2, false, map.params.zoomAnimate);
      //se déplacer en haut à gauche de la carte de manière animée
      setTimeout(() => {
        //changer la place de la carte
        _this.transX = 1000;
        _this.transY = 0;
      ...
      //consle log la position actuelle sur la carte
      console.log(_this.transX);
      console.log(_this.transY);

      //se déplacer en -300 en x et en -81 en y de manière animée
      setTimeout(() => {
